...
type Chapter = { id: string; chapterIndex: number; content: string; stageTitle?: string; stageSummary?: string }
type ChapterResponse = { book: Book; chapters: Chapter[] }
type Stage = { id: string; index: number; title: string; summary: string }

const fallbackStages = ['The Call to Adventure', 'Crossing the Threshold', 'Trials in the Wild', 'Allies and Enemies', 'The Hidden Truth', 'Shadows Deepen', 'The Inner Journey', 'The Final Challenge'].map((title, index) => ({ id: '', index: index + 1, title, summary: '' }))
...
function GoalCard({ stageCount }: { stageCount: number }) {
  return (
    <aside className="absolute right-3 top-3 z-20 w-40 rounded-2xl border border-white/10 bg-[#091011]/90 p-4 text-cream shadow-2xl backdrop-blur-md sm:right-6 sm:top-6 sm:w-48">
      <header className="flex items-center justify-between text-[10px]"><span>This quest</span><Target size={16} className="text-amber-300" /></header>
      <strong className="mt-5 block font-serif text-2xl">{stageCount}</strong>
      <span className="text-[10px] text-cream/35">stages to conquer</span>
      ...
      <p className="mt-4 text-[9px] text-cream/40">One stage, one step forward</p>
    </aside>
  )
}
...
function QuestMap({ stages }: { stages: Stage[] }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl bg-[#eadbc0] text-[#263027] shadow-[0_28px_90px_rgba(0,0,0,.35)]" style={{ minHeight: mapHeight, backgroundImage: "url('/assets/learning-map-clean.png')", backgroundSize: 'cover', backgroundPosition: 'center' }}>
      <div className="absolute inset-0 bg-[#efe1c7]/45 mix-blend-screen" /><GoalCard stageCount={stages.length} />
      ...
      {stages.map((stage, index) => {
        ...
        return (
          <article className="quest-node absolute z-10 flex items-center gap-4" style={{ left: `${nodeX[index % nodeX.length]}%`, top: 72 + index * 132, transform: 'translateX(-28px)', animationDelay: `${index * 70}ms` }} key={stage.index}>
            <button
              className={`relative grid h-14 w-14 shrink-0 place-items-center rounded-full border font-serif text-lg shadow-lg transition hover:scale-110 ${status === 'complete' ? 'border-[#294b32] bg-[#294b32] text-cream' : status === 'current' ? 'animate-[quest-pulse_2.2s_ease-in-out_infinite] border-orange-300 bg-orange-600 text-white shadow-[0_0_0_9px_rgba(238,157,70,.20),0_0_30px_rgba(229,139,72,.65)]' : 'border-[#c7b899] bg-[#f2e6cf] text-[#253028]'}`}
              type="button"
              aria-label={`Stage ${stage.index}, ${status}`}
              onClick={() => stage.id && navigate(`/reader/${stage.id}`)}
            >
            ...
            <div className="w-32 text-[9px] sm:w-44">
              <strong className={status === 'current' ? 'text-orange-700' : ''}>Stage {stage.index}</strong>
              <p className={`mt-1 text-[10px] leading-tight ${status === 'locked' ? 'blur-[3px] select-none' : ''}`}>
                {stage.summary || 'Continue your journey'}
              </p>
              <small className="mt-2 flex items-center gap-1 text-[#586157]">
...
export default function Path() {
  ...
  useEffect(() => {
    ...
    async function loadQuest() {
      ...
      setBook(data.book)
      if (data.chapters.length) setStages(data.chapters.map((chapter) => ({ id: chapter.id, index: chapter.chapterIndex, title: chapter.stageTitle || '', summary: chapter.stageSummary || '' })))
    }
    void loadQuest().catch(() => undefined)
    return () => controller.abort()
  }, [])

  return <main className="min-h-screen bg-[#070d10] text-cream"><div className="mx-auto grid max-w-[1280px] md:grid-cols-[220px_minmax(0,1fr)] lg:grid-cols-[300px_minmax(0,1fr)]"><JourneySidebar book={book} /><div className="p-3 sm:p-5 lg:p-7"><QuestMap stages={stages} /></div></div></main>
}
